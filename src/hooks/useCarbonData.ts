...
export const useCarbonData = () => {
  ...
  // Initialize demo data for 7 days
  ...
  const toggleChallenge = (challengeId: string) => {
    ...
    const updatedChallenges = current.challenges.map(challenge => {
      if (challenge.id === challengeId) {
        ...
        // If completed, add an entry reflecting the carbon saving offset
        let entries = [...current.entries];
        if (nextCompleted) {
          // add savings entry
          entries.push({
            id: `saving_${challengeId}_${Date.now()}`,
            optionId: 'saving_action', // custom id
            category: challenge.category,
            label: `Completed Challenge: ${challenge.title}`,
            value: 1,
            unit: 'action',
            co2e: -challenge.co2Saving,
            timestamp: new Date().toISOString()
          });
        } else {
          // remove savings entry
          entries = entries.filter(e => e.id !== `saving_${challengeId}_${Date.now()}` && !e.label.includes(challenge.title));
        }
  ...
  // Calculate Streak dynamically: consecutive days of staying under carbon budget or completing challenges
  useEffect(() => {
